class CompaniesController < ApplicationController
  ...
  def reports
    ...
    @reportdata = Driver.joins("INNER JOIN shipments ON shipments.driver_id = drivers.id and shipments.company_id = drivers.company_id").where("shipments.company_id" => @company.id).group("drivers.id").select("drivers.*, count(drivers.id) as driver_shipment_count")
